...
// Called every frame
void APNavGrid::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

FIntVector APNavGrid::GetGridTileCount() const
{
...
void APNavGrid::DrawDebug() const
{
	for (int X = 0; X < Grid.Num(); X++)
	{
		for (int Y = 0; Y < Grid[X]->SArray.Num(); Y++)
		{
			...
			{
				UPNavNode* CurrentGridNode = Grid[X]->SArray[Y]->FGridArray[Z];
...
void APNavGrid::DrawPath(TArray<UPNavNode*> PathToDraw, float Duration) const
{
	if (PathToDraw.Num())
	{
...
UPNavNode* APNavGrid::NodeFromWorldPoint(const FVector WorldPosition) const
{
...
TArray<UPNavNode*> APNavGrid::FindPath(const FVector StartPosition, const FVector TargetPosition)
{
	UPNavNode* StartNode = NodeFromWorldPoint(StartPosition);
	UPNavNode* TargetNode = NodeFromWorldPoint(TargetPosition);

	TArray<UPNavNode*> OpenSet;
	TArray<UPNavNode*> ClosedSet;

	auto Predicate = [](const UPNavNode& NodeA, const UPNavNode& NodeB)
	{
	...
	while (OpenSet.Num() > 0)
	{
		...
		for (UPNavNode* Neighbor : GetNeighbors(CurrentNode))
		{
...
TArray<UPNavNode*> APNavGrid::GetNeighbors(UPNavNode* CurrentNode)
{
	TArray<UPNavNode*> Neighbors;
...
int APNavGrid::GetDistance(UPNavNode* NodeA, UPNavNode* NodeB)
{
	int OutputValue = 0;
...
TArray<UPNavNode*> APNavGrid::RetracePath(UPNavNode* StartNode, UPNavNode* EndNode) const
{
	TArray<UPNavNode*> BuiltPath;
	UPNavNode* CurrentNode = EndNode;
	...
	for (int X = 0; X <= GridTileCount.X; X++)
	{
		...
		for (int Y = 0; Y <= GridTileCount.Y; Y++)
		{
			...
			for (int Z = 0; Z <= GridTileCount.Z; Z++)
			{
				...
				UPNavNode* NewNavNode = NewObject<UPNavNode>(this);
